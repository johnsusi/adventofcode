...
template <typename C> auto join(C &&container, std::string delim)
{
    using namespace std;
    ;
    ...
    advance(s, 1);
    return accumulate(s, e, init, [&](const auto &a, auto b) { return a + delim + to_string(b); });
}
...
auto getline(std::istream &in, std::string &line) -> std::istream &
{
    std::getline(in, line);
    if (!in.fail())
    {
